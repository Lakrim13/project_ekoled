...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if ($file_size > $max_size) {
            $_SESSION['error'] = "Le fichier est trop volumineux. Maximum 5MB.";
            header("Location: profile.php");
            exit();
        }
        ...
        $image = time() . "_" . bin2hex(random_bytes(8)) . "." . $file_ext;
...
if (isset($_GET['delete']) && isset($_GET['type'])) {
    ...
    $allowed_types = ['category' => 'categories', 'series' => 'series', 'product' => 'products'];
...
<a href="#" id="userIcon"><i class="fas fa-user-shield" style="background: linear-gradient(45deg, #f093fb, #f5576c); color: white; padding: 0.5rem 1rem; border-radius: 20px; font-size: 0.9rem; font-weight: 600;"></i></a>
</div>

<div class="user-menu" id="userMenu" style="background: linear-gradient(45deg, #f093fb, #f5576c); color: white; padding: 0.5rem 1rem; border-radius: 20px; font-size: 0.9rem; font-weight: 600;">
...
function updateStock(id, value) {
  ...
  if (!csrfToken) {
    showNotification('Erreur: Session expirée. Rechargez la page.', 'error');
    return;
  }
